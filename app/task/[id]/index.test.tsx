...
describe('S20 Manage Task Sheet', () => {
  ...
  it('renders "Undo snooze" when a prior day was vacated by a snooze pointing at today, and Undo restores it', async () => {
    const task = makeTask({ id: 'task-1' as Id });
    fake.seedTask(task);
    // Seed yesterday's row as already-snoozed into today directly (equivalent to having
    // performed the snooze on yesterday's own S20 instance) — daily cadence, so yesterday was
    // naturally due, and its own row now carries a pointer to today.
